...
final List<SingleChildWidget> providers = [
  ...
  Provider(
    create: (ctx) => NewsRepository(httpClient: ctx.read<IHttpClient>()),
  ),
  Provider(
    create: (ctx) => CommentsRepository(httpClient: ctx.read<IHttpClient>()),
  ),
  ChangeNotifierProvider<HomePageController>(
    create: (ctx) =>
        HomePageController(newsRepository: ctx.read<NewsRepository>()),
  ),
